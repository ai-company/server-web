use crate::{
    database::SqlPool,
    helper::{get_current_admin_id, get_current_user_id},
};
...
/// Route Authorization middleware
///
/// The route that contains this extractor will only execute if admin is logged on
///
/// Usage:
/// ```
/// fn route_handler(_: HttpRequest, _mw_auth: AdminAuthorized) -> impl Responder { ... }
/// ```
///
/// Flow:
/// * try get db connection
...
impl FromRequest for AdminAuthorized {
    ...
    fn from_request(_req: &HttpRequest, _payload: &mut dev::Payload) -> Self::Future {
        ...
        if let Some(pool) = pool {
            match get_current_admin_id(_req, pool) {
                Ok(_) => ok(AdminAuthorized),
                Err(_) => err(InternalError::from_response(
                    "unauthorized",
